...
while (list($id, $tkode, $tjudul, $tname, $tthn, $tgenre, $tstatus) = $otask->getResult()) {
	...
	// Tampilan jika status task nya belum dikerjakan
		...
		<td>" . $tstatus . "</td>
		<td>
		<button class='btn btn-danger'><a href='copy_index.php?id_hapus=" . $id . "' style='color: white; font-weight: bold;'>Hapus</a></button>
		<button class='btn btn-success' ><a href='copy_index.php?id_status=" . $id .  "' style='color: white; font-weight: bold;'>Selesai</a></button>
		</td>
		</tr>";
		$no++;
	}
